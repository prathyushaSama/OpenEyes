...
class Grammar():
 ...
 \
    ...
    GeneSymbol = Suppress('(') + (GeneProductID ^ AccNoFull('AccNoTransVar')) + Suppress(')')
    ...
    AccNo = NotAny('LRG_') + Combine(Word(unicode(alphas) + '_') + Number)('RefSeqAcc') + Optional(Version)
 ...
 \
    ...
    Offset = Word('+-', exact=1)('OffSgn') + Optional(Word('ud', exact=1))('OffOpt') + (Number ^ '?')('Offset')

    # BNF: RealPtLoc -> ((`-' | `*')? Number Offset?) | `?'

    RealPtLoc = Group((Optional(Word('-*', exact=1))('MainSgn') + Number('Main') + Optional(Offset)) ^ '?')

    # BNF: IVSLoc -> `IVS' Number (`+' | `-') Number

    IVSLoc = Group(Suppress('IVS') + Number('IVSNumber') + Word('+-', exact=1)('OffSgn') + Number('Offset'))('IVSLoc')
    ...
    RealExtent = Group(PtLoc('PtLoc'))('StartLoc') + Suppress('_') + Group(
        Optional(Group(Optional('o') + (RefSeqAcc ^ GeneSymbol) + Suppress(':') + Optional(RefType)))('OptRef') + PtLoc(
            'PtLoc'))('EndLoc')

    # BNF: EXLoc -> `EX' Number (`-' Number)?

    EXLoc = Group(Suppress('EX') + Number('EXNumberStart') + Optional(Suppress('-') + Number('EXNumberStop')))('EXLoc')
    ...
    FarLoc = (RefSeqAcc ^ GeneSymbol) + Optional(Suppress(':') + Optional(RefType) + Extent)

    # BNF: ChromBand -> (`p' | `q') Number `.' Number

    ChromBand = Suppress(Word('pq', exact=1)) + Number + Suppress('.') + Number

    # BNF: ChromCoords -> `(' Chrom `;' Chrom `)' `(' ChromBand `;' ChromBand `)'

    ChromCoords = Suppress('(') + Chrom + Suppress(';') + Chrom + Suppress(')') + Suppress('(') + ChromBand + Suppress(
        ';') + ChromBand + Suppress(')')
    ...
    Subst = Group(PtLoc('PtLoc'))('StartLoc') + Nt('Arg1') + Literal('>').setParseAction(replaceWith('subst'))(
        'MutationType') + Nt('Arg2')

    # BNF: Del -> Loc `del' (Nt+ | Number)?

    Del = Loc + Literal('del')('MutationType') + Optional(NtString ^ Number)('Arg1')

    # BNF: Dup -> Loc `dup' (Nt+ | Number)? Nest?

    Dup = Loc + Literal('dup')('MutationType') + Optional(NtString ^ Number)('Arg1') + Optional(Nest)

    # BNF: AbrSSR -> PtLoc Nt+ `(' Number `_' Number `)'

    AbrSSR = PtLoc + NtString + Suppress('(') + Number + Suppress('_') + Number + Suppress(')')

    # BNF: VarSSR -> (PtLoc Nt+ `[' Number `]') | (RangeLoc `[' Number `]') | AbrSSR

    VarSSR = (PtLoc + NtString + Suppress('[') + Number + Suppress(']')) ^ (
    RangeLoc + Suppress('[') + Number + Suppress(']')) ^ AbrSSR

    # BNF: Seq -> (Nt+ | Number | RangeLoc `inv'? | FarLoc) Nest?

    Seq = Group(NtString('Sequence') ^ Number ^ (RangeLoc('Range') + Optional(Literal('inv')('Inv'))) ^ FarLoc(
        'OptRef') + Optional(Nest))('Seq')
    ...
    Indel = (RangeLoc ^ Group(PtLoc('PtLoc'))('StartLoc')) + Literal('del') + Optional(NtString ^ Number)(
        'Arg1') + Literal('ins').setParseAction(replaceWith('delins'))('MutationType') + SimpleSeqList

    # BNF: Inv -> RangeLoc `inv' (Nt+ | Number)? Nest?

    Inv = RangeLoc + Literal('inv')('MutationType') + Optional(NtString ^ Number)('Arg1') + Optional(Nest)
    ...
    TransLoc = Suppress('t') + ChromCoords + Suppress('(') + FarLoc + Suppress(')')
    ...
    RawVar = originalTextFor((CRawVar ^ (Suppress('(') + CRawVar + Suppress(')'))) + Suppress(Optional('?')), False)
    ...
    UAlleleVarSet = (CAlleleVarSet ^ (Suppress('(') + CAlleleVarSet + Suppress(')'))) + Suppress(Optional('?'))

    # BNF: SimpleAlleleVarSet -> (`[' UAlleleVarSet `]') | ExtendedRawVar

    SimpleAlleleVarSet << (Group(Suppress('[') + UAlleleVarSet + Suppress(']') ^ ExtendedRawVar)('SimpleAlleleVarSet'))

    # BNF: MosaicSet -> (`[' SimpleAlleleVarSet (`/' SimpleAlleleVarSet)* `]') | SimpleAlleleVarSet

    MosaicSet = Group(
        Suppress('[') + SimpleAlleleVarSet + ZeroOrMore(Suppress('/') + SimpleAlleleVarSet) + Suppress(']'))(
        'MosaicSet') ^ SimpleAlleleVarSet

    # BNF: ChimeronSet -> (`[' MosaicSet (`//' MosaicSet)* `]') | MosaicSet

    ChimeronSet = Group(Suppress('[') + MosaicSet + ZeroOrMore(Suppress('//') + MosaicSet) + Suppress(']'))(
        'ChimeronSet') ^ MosaicSet

    # BNF: SingleAlleleVarSet -> (`[` ChimeronSet ((`;' | `^') ChimeronSet)*

    #                            (`(;)' ChimeronSet)* `]') | ChimeronSet

    SingleAlleleVarSet = Group(
        Suppress('[') + ChimeronSet + ZeroOrMore((Suppress(';') ^ Suppress('^')) + ChimeronSet) + ZeroOrMore(
            Suppress('(;)') + ChimeronSet) + Suppress(']'))('SingleAlleleVarSet') ^ ChimeronSet
    ...
    MultiAlleleVars = DRef + Group(SingleAlleleVarSet + OneOrMore(Suppress(';') + SingleAlleleVarSet))(
        'MultiAlleleVars')
    ...
    MultiTranscriptVar = DRef + Suppress('[') + ExtendedRawVar + ZeroOrMore(Suppress(';') + ExtendedRawVar) + OneOrMore(
        Suppress(',') + ExtendedRawVar + ZeroOrMore(Suppress(';') + ExtendedRawVar)) + Suppress(']')
    ...
    AA3 = Literal('Ala') ^ Literal('Arg') ^ Literal('Asn') ^ Literal('Asp') ^ Literal('Cys') ^ Literal('Gln') ^ Literal(
        'Glu') ^ Literal('Gly') ^ Literal('His') ^ Literal('Ile') ^ Literal('Leu') ^ Literal('Lys') ^ Literal(
        'Met') ^ Literal('Phe') ^ Literal('Pro') ^ Literal('Ser') ^ Literal('Thr') ^ Literal('Trp') ^ Literal(
        'Tyr') ^ Literal('Val') ^ Literal('Ter')
    ...
    (Literal('Met1') ^ Literal('M1')) + (
    Literal('?') ^ (Literal('ext') + Literal('-') + Number)))) + Empty().setParseAction(replaceWith('subst'))(
        'MutationType')
    ...
    PCRawVar = Group(
        PSubst ^ PDel ^ PDup ^ PVarSSR ^ PIns ^ PIndel ^ FrameShift ^ Literal('=') ^ Literal('?') ^ Literal(
            '0') ^ Literal('0?'))('RawVar')
    ...
    OneOrMore(Suppress(';') + PRawVar) ^ OneOrMore(Suppress(',') + PRawVar)) + Suppress(']')
